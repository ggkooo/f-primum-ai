  ...
  return (
    <aside className="hidden h-full w-72 shrink-0 flex-col border-r border-zinc-900/10 bg-white/30 shadow-[inset_-1px_0_0_rgba(255,255,255,0.28)] backdrop-blur-md md:flex">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        <p className="mb-1.5 px-3 text-[10px] font-semibold uppercase tracking-widest text-zinc-400">Recentes</p>
        {isLoading && chats.length === 0 ? (
          <div className="px-3 py-4 text-sm text-zinc-500">Carregando conversas...</div>
        ) : null}
        ...
        {!isLoading && !error && chats.length === 0 ? (
          <div className="px-3 py-4 text-sm text-zinc-500">Nenhuma conversa ainda.</div>
        ) : null}
        ...
        {chats.length > 0 ? (
          <div className="space-y-0.5">
            {chats.map((chat) => (
              ...
              >
                <span className="truncate text-sm text-[#26312b]">{chat.title || 'Conversa sem título'}</span>
                <span
